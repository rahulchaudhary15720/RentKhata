...
type Item = { id: number; ownerId: number; name: string; category: string; unit: string; quantity: number; minimumStock: number; unitPrice: number; purchasedBy: string; expiryDate: string | null; notes: string; updatedAt: string };
type Transaction = { id: number; itemId: number; type: "Stock in" | "Stock out" | "Correction"; quantityChange: number; personName: string; note: string; createdAt: string };
type Owner = { id: number; name: string; email: string };
type Data = { items: Item[]; transactions: Transaction[]; owners: Owner[] };
...
export default function GroceryManager({ search }: { search: string }) {
  const [data, setData] = useState<Data>({ items: [], transactions: [], owners: [] });
  ...
  const submitItem = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); void post({ action: editing ? "updateItem" : "addItem", id: editing?.id, ownerId: form.get("ownerId") ? Number(form.get("ownerId")) : undefined, name: form.get("name"), category: form.get("category"), unit: form.get("unit"), quantity: Number(form.get("quantity")), minimumStock: Number(form.get("minimumStock")), unitPrice: Number(form.get("unitPrice")), purchasedBy: form.get("purchasedBy"), expiryDate: form.get("expiryDate") || null, notes: form.get("notes") }, editing ? "Grocery item updated." : "Grocery purchase added."); };
  const submitStock = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!stockItem) return; const form = new FormData(event.currentTarget); void post({ action: "adjustStock", id: stockItem.id, direction, quantity: Number(form.get("quantity")), purchasedBy: form.get("purchasedBy"), note: form.get("note") }, direction === "in" ? "Purchase added to stock." : "Stock usage recorded."); };
  ...
  const filtered = data.items.filter((item) => (filter === "All" || item.category === filter) && `${item.name} ${item.category} ${item.purchasedBy} ${data.owners.find(owner => owner.id === item.ownerId)?.name ?? ""} ${item.notes}`.toLowerCase().includes(search.trim().toLowerCase()));
  ...
  return <>
    ...
    <section className="page-panel grocery-panel">
      ...
      {loading ? <AppLoader label="Loading grocery inventory" /> : tab === "inventory" ? <>
        ...
        <div className="table-wrap"><table><thead><tr><th>ITEM</th>{data.owners.length > 0 && <th>OWNER</th>}<th>PURCHASED BY</th><th>CATEGORY</th><th>ON HAND</th><th>VALUE</th><th>EXPIRY</th><th>ACTIONS</th></tr></thead><tbody>
          {filtered.map(item => { const isLow = item.quantity <= item.minimumStock; const expired = item.expiryDate && new Date(`${item.expiryDate}T00:00:00`) < today; return <tr key={item.id}><td><strong>{item.name}</strong>{item.notes && <small className="table-sub">{item.notes}</small>}</td>{data.owners.length > 0 && <td>{ownerMap.get(item.ownerId)?.name ?? "Unknown"}</td>}<td><strong>{item.purchasedBy || "Not recorded"}</strong></td><td>{item.category}</td><td><strong>{number.format(item.quantity)} {item.unit}</strong> {isLow && <span className="status overdue">Low</span>}<small className="table-sub">Minimum: {number.format(item.minimumStock)} {item.unit}</small></td><td>{money.format(item.quantity * item.unitPrice)}</td><td>{item.expiryDate ? <span className={expired ? "expiry expired" : "expiry"}>{date(item.expiryDate)}</span> : "—"}</td><td><div className="row-actions"><button onClick={() => { setStockItem(item); setDirection("in"); setDialog("stock"); }}>Stock</button><button onClick={() => { setEditing(item); setDialog("item"); }}>Edit</button><button className="danger-button" onClick={() => { if (window.confirm(`Delete ${item.name} and its stock history?`)) void post({ action: "deleteItem", id: item.id }, "Grocery item deleted."); }}>Delete</button></div></td></tr>; })}
    ...
    {dialog && <div className="modal-layer" role="dialog" aria-modal="true"><button className="modal-backdrop" aria-label="Close dialog" onClick={close} /><div className="modal-card"><div className="modal-head"><div><span className="section-kicker">GROCERY MANAGEMENT</span><h2>{dialog === "item" ? editing ? "Edit grocery item" : "Add grocery item" : `Update ${stockItem?.name}`}</h2></div><button className="close" onClick={close}>×</button></div>
      {dialog === "item" ? <form className="form-grid" onSubmit={submitItem} key={editing?.id ?? "new"}>
        ...
        <Field label="Purchased by"><input name="purchasedBy" required maxLength={80} defaultValue={editing?.purchasedBy} placeholder="Person who purchased this item" /></Field>
        {data.owners.length > 0 && !editing && <Field label="Record owner"><select name="ownerId" required defaultValue=""><option value="" disabled>Select user account</option>{data.owners.map(owner => <option key={owner.id} value={owner.id}>{owner.name} — {owner.email}</option>)}</select></Field>}
      ...
      </form> : <form className="form-grid" onSubmit={submitStock}><div className="stock-choice"><button type="button" className={direction === "in" ? "active" : ""} onClick={() => setDirection("in")}>+ Stock in</button><button type="button" className={direction === "out" ? "active" : ""} onClick={() => setDirection("out")}>− Stock out</button></div><div className="stock-current">Current stock: <strong>{number.format(stockItem?.quantity ?? 0)} {stockItem?.unit}</strong></div><Field label="Quantity"><input name="quantity" type="number" min="0.01" max={direction === "out" ? stockItem?.quantity : 999999999} step="0.01" required autoFocus /></Field>{direction === "in" && <Field label="Purchased by"><input name="purchasedBy" required maxLength={80} defaultValue={stockItem?.purchasedBy} placeholder="Person who purchased this stock" /></Field>}<Field label="Reason / note"><input name="note" maxLength={300} placeholder={direction === "in" ? "Store, bill reference…" : "Used, spoiled…"} /></Field><Actions saving={saving} onCancel={close} label={direction === "in" ? "Add purchase" : "Record usage"} /></form>}
    </div></div>}
  </>;
}
