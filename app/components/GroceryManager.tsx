...
type Item = { id: number; name: string; category: string; unit: string; quantity: number; minimumStock: number; unitPrice: number; purchasedBy: string; expiryDate: string | null; notes: string; updatedAt: string };
...
export default function GroceryManager({ search }: { search: string }) {
  ...
  const filtered = data.items.filter((item) => (filter === "All" || item.category === filter) && `${item.name} ${item.category} ${item.purchasedBy} ${item.notes}`.toLowerCase().includes(search.trim().toLowerCase()));
