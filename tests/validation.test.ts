...
const validItem = { name: "Rice", category: "Grains", unit: "kg", quantity: 10, minimumStock: 2, unitPrice: 70, purchasedBy: "Rahul", expiryDate: "2027-02-28", notes: "Dry shelf" };
...
test("grocery validation rejects blank and oversized text", () => {
  ...
  assert.match(validateGroceryItem({ ...validItem, purchasedBy: " " })!, /purchaser/i); assert.match(validateGroceryItem({ ...validItem, purchasedBy: "x".repeat(81) })!, /purchaser/i);
});
